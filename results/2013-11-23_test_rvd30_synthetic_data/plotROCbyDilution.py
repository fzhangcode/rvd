...
def main():
    ...
    folderList = ('2013-11-23_test_rvd30_synthetic_data/10',\
                    '2013-11-23_test_rvd30_synthetic_data/100',\
                    '2013-11-23_test_rvd30_synthetic_data/1000',\
                    '2013-11-23_test_rvd30_synthetic_data/10000')
...
def ROCpoints(controlFile,caseFile, path, d,N=1000, P=0.95, chi2=False):
    ...
    (Z, caseMuS, controlMuS) = rvd30.sample_post_diff(caseMu-casePhi['mu0'], controlMu-controlPhi['mu0'], N)
    ...
    pList = [rvd30.bayes_test(Z, [(t, np.inf)]) for t in T]
    ...
    if chi2:
        ...
        chi2P = np.zeros(J)
        for j in xrange(J):
                chi2Prep[j,:] = np.array([rvd30.chi2test( caseR[i,j,:] ) for i in xrange(nRep)] )
