...
Rij->Qij -> mu -> mu0
         ...
         -> M  -> a
	       -> b
"""
"""rvd28.py: Compute MAP estimates for RVD2.8 model."""
...
def write_vcf(outputFile, loc, call, refb, altb, caseMu, controlMu):
    ...
    for i in xrange(J):
        if call[i]:
            print("%s\t%d\t.\t%c\t%s\t.\tPASS\tAF=%0.3f;CAF=50.3f" % (chrom[i], pos[i], refb[i], altb[i], caseMu[i]*100., controlMu[i]*100.0), file=vcfF)
...
def save_model(h5Filename, phi, mu=None, theta=None, r=None, n=None, loc=None, refb=None):
    ...
    h5Filename = Temp_Directroy + '/' + h5Filename
...
def generate_sample(a=2.5e4, b=5, mu0=0.25, M0=10, n=100, N=3, J=100, seedint=None):
...
def estimate_mom(r, n):
    ...
    a = (m1*m1)/(m2 - m1*m1 + np.finfo(np.float).eps)
    
    b = (m2 - m1*m1)/(m1 + np.finfo(np.float).eps)
...
def sampleLocMMH(args):
    ...
    logPM_p = np.sum(beta_log_pdf(M, alpha_p, beta_p)) \
                + ss.gamma.logpdf(M_p, a, scale=b)
                    
    # Log-likelihood for the current M
    alpha = mu*M + np.finfo(np.float).eps
    beta = (1-mu)*M + np.finfo(np.float).eps
    logPM = np.sum(beta_log_pdf(M, alpha, beta)) \
...
def mh_sample(r, n, gibbs_nsample=10000, mh_nsample=10, burnin=0.2, thin=2, pool=None):
    ...
    h5file['phi'].create_dataset('b', (1,), dtype='f')
    ...
    for i in xrange(gibbs_nsample):
        ...
        M_mh = sampleMMH(M, phi['a'], phi['b'], mu=mu, mh_sample=mh_sample, pool=pool)
    ...
    if burnin > 0.0:
        ...
        # identity the paramter , assump 3
        M_s = np.deleta(M_s, np.s_[0:np.int(burnin*gibbs_nsample):], 3 )
    if thin > 1:
        mu_s = np.delete(mu_s, np.s_[::thin], 1)
        theta_s = np.delete(theta_s, np.s_[::thin], 2)
        # identity the paramter , assump 3
        M_s = np.deleta(M_s, np.s_[::thin], 3)
