...
void export_depthchart_Title(FILE* fid)
{
...
void export_depthchart(const pile_t *pile, const unsigned long numLines, const char *chr, FILE* fid)
{
    //char *chr="chr1";
    ...
    for (i=0; i<numLines; i++) {
        fprintf(fid, "%d\t%s\t%d\t%s\t%c", i+1, chr, pile[i].pos, chr, baseChar[pile[i].refb]);
        fprintf(fid, "\t");
        fprintf(fid, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%c", pile[i].depthT[A], pile[i].depthT[C], 
        ...
        fprintf(fid, "\t");
        fprintf(fid, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%c", pile[i].depthF[A], pile[i].depthF[C], 
        ...
        fprintf(fid, "\t");
        fprintf(fid, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%c", pile[i].depthR[A], pile[i].depthR[C], 
...
int main (int argc, char *argv[])
{
	...
	export_depthchart_Title(plfid2);
    ...
    for (i=0; i<numLines; i++) {
		...
		export_depthchart ( pile, 1, chrName, plfid2);
    }
    fclose(plfid);
	fclose(plfid2);
