...
try:
    ...
    rvd30.save_model(h5FileName, phi, mu=mu_s, theta=theta_s, M=M_s, r=r, n=n, loc=loc, refb=refb)
...
for dilution in np.unique(toc[toc.isRef=='N'].Dilution):
    ...
    try:
        ...
        phi, theta_s, mu_s, M_s = rvd30.mh_sample(r, n, gibbs_nsample=gibbs_nsample,mh_nsample=mh_nsample, burnin=0.2, pool=pool)
        logging.debug("Saving model in %s" % h5FileName)
        rvd30.save_model(h5FileName, phi, mu=mu_s, theta=theta_s, M=M_s, r=r, n=n,loc=loc,
           refb=refb)
