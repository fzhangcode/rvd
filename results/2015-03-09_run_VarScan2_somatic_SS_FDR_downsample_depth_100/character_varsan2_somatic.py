...
def main():    
    ...
    for k, v in method.iteritems():
        ...
        for d in DilutionList:
            ...
            for r in DepthList:
                ...
                hdf5Dir='./%s' %str(r)
                caseFile = '../2015-02-23_run_rvd2_SS_FDR_downsample_depth_100/Case%s.hdf5' %str(d).replace('.','_')
                #caseFile = "%(dir)s/%(file)s" %{'dir':hdf5Dir,'file':caseFile}
                print caseFile
                #(_, _, _, _, _, _, caseN, _) = rvd27.load_model(caseFile)
                (_, _, _, _, _, caseN, _) = load_model(caseFile)
    ...
    book.save('character_varsan2_somatic.xls') 
