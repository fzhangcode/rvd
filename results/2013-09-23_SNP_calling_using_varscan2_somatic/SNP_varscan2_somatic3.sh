...
do
	...
	echo -------------------------------------------------------
	echo pileup $PILEUPDIR/control.bam 
	if [ -f $PILEUPDIR/control.pileup ]
		then
			echo File $PILEUPDIR/control.pileup exists already
		else
			samtools mpileup -C 50 -d 100000 -f $FASTAFILE $PILEUPDIR/control.bam  > $PILEUPDIR/control.pileup 
	fi
	...
	if [ -f $PILEUPDIR/case100_0.pileup ]
		then
			...
			samtools mpileup -C 50 -d 100000 -f $FASTAFILE $PILEUPDIR/case100_0.bam  > $PILEUPDIR/case100_0.pileup 
	fi
