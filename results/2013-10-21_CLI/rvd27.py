...
def write_vcf(outputFile, loc, call, refb, altb, caseMu):
    ...
    print("##INFO=<ID=AF,Number=A,Type=Float,Description=\"Allele Frequency\">", file=vcfF)
...
def generate_sample(M, M0=10, mu0=0.0025, n=100, N=3, J=100, seedint=None):
    """Returns a sample with n reads, N replicates, and
    J locations. The parameters of the model are M0, mu0, Mj.
    """
    ...
    if M is None: 
        M = [100 for i in range(J)]
    """else:
        assert len(M) == J, "length of M doesn't match J"""
