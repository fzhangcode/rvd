...
def main():
    ...
    #logging.debug("Processing dilution: %s" % str(d))
    	for d in dilutionList:
            ...
            txtfile='work/%(frac)s/vcf%(dilution)s.vcf' \
            %{'frac':str(f),'dilution':str(d).replace('.','_')}
            ##pdb.set_trace()
            # import the call_stats file
            callstatus= pd.read_table(txtfile, header=38, error_bad_lines=False)
            ...
            for i in xrange(len(pos)):
                    print("%s\t%d\t.\t%s\t%s\t.\tPASS\t%0.1f" % (chrom, pos[i], ref[i], alt[i], 0), file=vcfF)            
            vcfF.close()
