...
def main():
    ...
    muCaseP = Mu_distr(caseMu, [(0, 1.0/3),(1.0/3,2.0/3),(2.0/3,1)])
    muControlP = Mu_distr(controlMu, [(0, 1.0/3),(1.0/3,2.0/3),(2.0/3,1)])
    ...
    type={
        (0,0):'Reference',
        (0,1):'Heterozygote mutation',
        (0,2):'Homozygote mutation',
        (1,0):'Loss of Heterozygote',
        (1,1):'Germline',
        (1,2):'Loss of Heterozygote',
        (2,0):'Homozygote mutation',
        (2,1):'Heterozygote mutation',
        (2,2):'Unknow',
        }
    mutation=[]
    for i in xrange(len(muCaseP)):
        mutation.append(type[(CaseMaxpIdx[i],ControlMaxpIdx[i])])
    pdb.set_trace()
    mutation=np.array(mutation)

        
    
    #for caseP, controlP in zip(muCaseP,muControlP):
    ...
    contdc=pd.read_table('../2013-10-30_HCC1187_TP53/depth_chart/HCC1187BL_S1.dc')
    casedc=pd.read_table('../2013-10-30_HCC1187_TP53/depth_chart/HCC1187C_S1.dc')
    ...
    with open('result.txt','w') as f:
        print >> f, 'count\tloc\tpostP\tsomatic_status\trefb\tA0\tC0\tG0\tT0\tA1\tC1\tG1\tT1'
        for f0, f1, f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12 in zip(idx,loc[idx], postP[idx],mutation[idx],refb[idx],
                                                       A0[idx],C0[idx],
                                                       G0[idx],T0[idx],
                                                       A1[idx],C1[idx],
                                                       G1[idx],T1[idx],):
            print >> f, f0,'\t', f1,'\t',f2,'\t',f3,'\t',f4,'\t',f5,'\t',f6,'\t',f7,'\t',f8,'\t',f9,'\t',f10,'\t',f11,'\t',f12
        f.close()
