...
def main():
    ...
    figtitleopt = ('OptimalT_vs_Dilution5_linear','OptimalT_vs_Dilution4_linear')
    ...
    for m in xrange(2):
        ...
        for k in xrange(len(folderList)):
            ...
            ax.plot(dilutionList,optT_avg[k],'b*-',label=str(cov[k]))
            for i in xrange(len(dilutionList)):
                
                ax.plot([dilutionList[i],dilutionList[i]],[optT_avg[k][i]-optT_std[k][i],optT_avg[k][i]+optT_std[k][i]],color='r')
            if m==0 and k!=0:
