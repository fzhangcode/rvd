...
def generate_sample(a=1, b=2, mu0=0.0025, M0=10, n=100, N=3, J=100, seedint=None):
...
def estimate_mom(r, n):
    ...
    a = (m1*m1)/(m2 - m1*m1 + np.finof(np.float).eps)
    
    b = (m2 - m1*m1)/(m1 + np.finof(np.float).eps)
