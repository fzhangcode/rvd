...
& \qquad+(u_0M_0-1)\left( \psi(\alpha_{kj}) - \psi(\alpha_{kj} + \beta_{kj}) \right) + ((1-u_0)M_0-1)\left( \psi(\beta_{kj}) - \psi(\alpha_{kj} + \beta_{kj}) \right)
\end{split}
\end{align}
...
\begin{multline}
\mathcal{L}_{[u_0]} = K\sum_{j=1}^J\left(-\frac{1}{2}(\gamma_{1j}-u_0)^2\right) \\+ \sum_{j=1}^J \sum_{k=1}^Ku_0M_0\left( \psi(\alpha_{kj}) - \psi(\alpha_{kj}+\beta_{kj}) \right) - \sum_{j=1}^J \sum_{k=1}^Ku_0M_0\left( \psi(\beta_{kj}) - \psi(\alpha_{kj}+\beta_{kj}) \right)
\end{multline}

Taking the derivative with respect to $u_0$ gives
\begin{equation}
\frac{\partial \mathcal{L}}{\partial u_0} = K\sum_{j=1}^J\gamma_{1j} - KJu_0 +M_0 \sum_{j=1}^J \sum_{k=1}^K \left( \psi(\alpha_{kj}) - \psi(\beta_{kj}) \right)
\end{equation}

Solving for $u_0$ gives
\begin{equation}
u_0 \leftarrow \frac{1}{J}\sum_{j=1}^J \gamma_{1j} +\frac{M_0}{KJ}\sum_{j=1}^J \sum_{k=1}^K \left( \psi(\alpha_{kj}) - \psi(\beta_{kj}) \right)
\end{equation}
