...
We used RVD2 to identify germline and somatic mutations in the diploid HCC1187 sample. To identify germline mutations, we used the control data in the following hypothesis test $\Pr( \tilde{\mu}_j^{\text{control}} \in [\tau_\text{l},\tau_\text{u}] ) > 1-\alpha$, where the intervals are: homozygous reference $[0, 0.05)$, heterozygous mutant $[0.5, 0.0.75)$, and homozygous mutant $[0.75, 1.0]$.
...
\begin{figure*}[!tpb]
\begin{center}
\includegraphics[width=0.7\textwidth]{pdf_figs/HCC1187_MuBarPlot.pdf}
\caption{Estimated minor allele fraction for called variants in PAXIP1 gene. Star (*) indicates $\mu^{\text{case}}$  is significantly different from $ \mu^{\text{control}}$.}
\label{fig:brca_MAF}
\end{center}
\end{figure*}
%('rvd2\results\2013-11-21_HCC1187_PAXIP1_mu_barplot_T0')
%TODO: add star to test that have mudelta > .16 Done.
%TODO: set tau=0.16 for mudelta test.  Done

\begin{figure*}[!tpb]
    \centering
    \subfloat[Estimated minor allele fraction for called variants in PAXIP1 gene.  $ \mu^{\text{control}}$.]{{\includegraphics[width=0.8\textwidth]{pdf_figs/HCC1187_dotplot.pdf} }}%
    %
    \subfloat[Depth Chart for Positions called by RVD2 and muTect]{{\includegraphics[width=0.2\textwidth]{pdf_figs/position_lookup_chart.pdf} }}%
