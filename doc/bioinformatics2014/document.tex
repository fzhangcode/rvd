...
\section{Results:}We present a novel variant calling algorithm that uses a hierarchical Bayesian model to estimate allele frequency and call variants in heterogeneous samples. We show that our algorithm improves upon current classifiers and has higher sensitivity and specificity over a wide range of median read depth and minor allele frequency. We identify twelve mutations in the PAXP1 gene in a matched clinical breast ductal carcinoma tumor sample; two of which are loss-of-heterozygosity events.
...
Equation~\eqref{eqn:fisher_combined} gives a test statistic that follows a $\chi^2$ distribution with $2N$ degrees of freedom when the null hypothesis is true. If the sample average is higher than 200, we use the Benjamini-Hochberg method to control the family-wise error rate (FWER) over positions that have been called by the posterior distribution test \citep{benjamini1995controlling, efron2010large}.
%TODO: Bonferroni correction, set an approapriate threshold for read depth.
%%%%%%%%%%%%%%%%%%
% Data Sets
%%%%%%%%%%%%%%%%%%
\section{Data Sets}
...
The statistics for RVD is an average statistics as RVD uses six control replicates to estimate the model but returns statistics separately for three set of pair-end case replicates. RVD performed the best among all algorithms when the read depth is as high as 40000x. RVD correctly called all the mutated positions across all MAF levels with no false positives when MAF level is 0.3\% or lower. RVD tends to be a little bit over sensitive when the MAF level is 1.0\% or higher, which causes specificity slightly lower than 1.00. However, RVD program fails when the depth is unable to measure the baseline error rate and is not able to call any mutations.
