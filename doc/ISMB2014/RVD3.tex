...
Playing no prior on M is an implicit improper prior.

\subsubsection{Jeffreys Prior}
Jeffreys Prior:Jeffreys prior is proposed for invariance by Harold Jeffreys (1946), and defined in terms of the Fisher information. For our ploblem,
...
In order to examine the robustness of RVD3 to changes of different priors, we perform information prior (Log-normal) and noninformation prior (Jeffreys).
...
We generated receiver-operating characteristic curves (ROCs) for a range of median read depth and a range of minor allele frequencies (MAFs). For these ROC curves, we used the Bayesian test without the $\chi^2$ test. Adding the $\chi^2$ improves specificity at the expense of sensitivity. Figure~\ref{fig:ROC_jeffrey} and Figure~\ref{fig:ROC_lognormal} shows ROC curves generated by varying the threshold $\tau$ with a fixed $\alpha=0.05$. Figure~\ref{fig:ROC}A shows ROC curves for a true 0.1\% MAF for a range of median coverage depths. At the lowest depth the sensitivity and specificity is no better than random. However, we would not expect to be able to call a 1 in 1000 variant base with a coverage of only 43. The performance improves monotonically with read depth. Figures~\ref{fig:ROC_jeffrey}B-C show a similar relationship between coverage depth and accuracy for higher MAFs.
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=160mm]{tables/FDR_ACC_MCC.pdf}
\caption{Sensitivity/Specificity comparison of RVD3 with different priors.}
\label{tbl:FDR_ACC_MCC}
\end{center}
\end{figure}

Figure~\ref{tbl:SS} shows that the sensitivity and specificity of the RVD3 with different priors. Log-normal prior shows a higher sensitivity and specificity value than the Jeffreys prior.
Figure~\ref{tbl:FDR_ACC_MCC} shows the false discovery rate, accuracy, anc corelation coefficient of the RVD3 with different priors. Jeffreys prior shows a smaller false discovery rate and higher accuracy.
Based on the various advantages for Jeffreys and log-normal prior, RVD3 can afford a more appropariate choice for the precision prior. Here for we chose Jeffreys prior because it needs more attention to false discovery rate and accuracy for variants calling research, compared with the clinical experiment or diagnosis which cares more on true positive rate and true negative rate.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
