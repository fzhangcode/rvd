...
(RVD3) Sensitivity analysis Bayesian model
...
%TODO: [DISCUSS \citet{Stead:2013fu}] and others
...
(RVD3)
Improper Prior: Playing no prior on M is an implicit improper prior.
...
(RVD3) For Jeffreys prior,since ${\theta }_{ji}\sim Beta\left( {\mu }_{j},{M}_{j}\right)$, the Fisher information is given by
...
In order to examine the robustness of RVD3 to changes of different priors, we...

We found that RVD3 is robust to changing the priors.
The RVD3 algorithm provides estimates of model parameters and latent variables given the data. We show several of these parameters in Figure~\ref{fig:M_loc_rvd29} and Figure ~\ref{fig:M_loc_rvd30}.
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=120mm]{figs/ROC_without_chi2_rvd30.pdf}
\caption{ROC curve varying read depth showing detection performance.}
\label{fig:ROC_30}
\end{center}
\end{figure}

Empirical performance compared with other algorithms:
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=160mm]{tables/comparison_table_ss.pdf}
