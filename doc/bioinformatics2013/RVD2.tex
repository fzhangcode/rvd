...
A candidate sample is generated from the symmetric proposal distribution $Q(\mu_j^* | \mu_j^{(p)}) \thicksim \mathcal{N}(\mu_j^{(p)}, \sigma_j^2)$, where $\mu_j^{(p)}$ is the $p$th from ythe posterior distribution. The acceptance probability is then
\begin{equation}
...
%TODO: make sure \sigma_j is correct here.
...
The HCC1187 cell line is derived from  epithelial cells from primary breast tissue from a 41 y/o adult with TNM stage IIA primary ductal carcinoma. A matched normal cells were derived from lymphoblastoid cells from whole blood~\citep{}. 
% TODO: cite the original publication here.
...
%TODO: add start and stop location here
...
%TODO: verify these rs positions
...
The original research describing this sample used Strelka to identify mutations. They identified chr7:154760439 as variant, but did not call the other four variants. In particular Strelka missed the two LOH events.
...
%TODO: review grammar and completeness here
