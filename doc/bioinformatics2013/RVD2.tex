...
\begin{abstract}
Next-generation sequencing technology is increasingly being used for clinical diagnostic tests. Clinical samples are often genomically heterogeneous due to low sample purity or the presence of genetic subpopulations. However, many variant calling algorithms are optimized to call single nucleotide polymorphisms in homogeneous rather than heterogeneous samples. We present a novel variant calling algorithm that uses a hierarchical Bayesian model to estimate allele frequency and call variants in heterogeneous samples. We show that our algorithm improves upon current classifiers and has higher sensitivity and specificity over wide ranges of median read depth and a minor allele frequency. We identify XX mutations in the PAXXX gene in a matched clinical XXX tumor sample with 80\% tumor purity and XX median read depth.
...
We have found that the algorithm performance improves when we take the median of five or more M-H samples as a single Gibbs step for each position. We set the proposal distribution variance $\sigma_j^2 = 0.1 \cdot \mu_j^{(p)}$ if $\mu_j^{(p)} >0$ and $0.1$ otherwise. We resample from the proposal if the sample is outside of the support of the posterior distribution. In the experiments, we discard 20\% of the sample for burn-in and thin the chain by a factor of 2 to reduce autocorrelation among samples. Since, each position $j$ is exchangeable given the global hyperparameters $\mu_0$ and $M_0$ this algorithm can be distributed across $J$ processors. 
%%%%%%%%%%%%%%%%%%
...
We generated receiver-operating characteristic curves (ROCs) for a range of median read depth and a range of minor allele frequencies (MAFs). We used DNA sequenced data from two chemically synthesized DNA segments with known differential bases and positions as our test data set.

Two 400bp DNA sequences that are identical except at 14 loci with variant bases were synthesized and clonally isolated and labeled case and control. Sample of the case and control DNA were mixed at defined fractions to yield minor allele frequencies (MAFs) of 0.1\%, 0.3\%, 1\%, 10\%, and 100\%. More details of the experimental protocol are available from the original publication~\cite{}.

<<<<<<< HEAD
BAM files for the synthetic DNA data were down sampled by $10\times$, $100\times$, $1,000\times$, $10,000\times$ using Picard Version 1.96 to simulate lower coverage data while retaining the error structure of real NGS data. The final data set contains pairs of reads for three replicates of each case and a pair of reads for the control sample giving a total of 36 BAM files for each downsampled data set.
=======
To simulate lower coverage data while retaining the error structure of real NGS data, BAM files for the synthetic DNA data were downsampled $10\times$, $100\times$, $1,000\times$, and $10,000\times$. The final data set contains read pairs for three replicates of each case and pairs of reads three replicates for the control sample giving $N=6$ replicates for the control and each MAF level.
>>>>>>> edits to manuscript.

Figure~\ref{fig:ROC} shows ROC curves generated by varying the threshold $\tau$ with a fixed $\alpha=0.05$. Figure~\ref{fig:ROC}A shows ROC curves for a true 0.1\% MAF for a range of median coverage depths. At the lowest depth the sensitivity and specificity is no better than random. However, we would not expect to be able to call a 1 in 1000 variant base with a coverage of only 43. The performance improves monotonically with read depth. Figures~\ref{fig:ROC}B-C for larger MAFs show a similar relatonship between coverage depth and accuracy.
...
We compare the empirical performance of RVD2 to other variant calling algorithms using the synthetic DNA data sets described in the previous section. In a research application, the false discovery rate is more relevant performance metrics because the aim is generally to identify interesting variants. The sensitivity/specificity metric is more relevant in clinical applications where one is more interested in calling all of the true variants and none of the true negatives.

<<<<<<< HEAD
\subsubsection*{Parameters setting for variant calling}\

The variant calling algorithms we compare RVD2 to include samtools/bcftools, GATK VarScan2-mpileup, Varscan2-somatic, Strelka, and muTect. 

\paragraph{\textbf{Samtools}}We used samtools/bcftools (v0.1.19) for variant calling. mpileup was the function to call variants and bcftools was used to save the result in standard Variant Call Format (VCF) files. In mpileup, we set the -d option sufficiently high at $10^6$ to avoid coverage cap. Our synthetic data was mapped using BWA Version 0.7.4, -C50 option was applied to minimize the effect from reads with overestimated mapping quality. mpileup was able to accept multiple replicates at one time so six bam files from each replicate group were fed into mpileup for variant calling. Read depth for each sample was kept with -D enabled, and -u was turned on to make sure the output bcf files uncompressed. 
...
Samtools performs very well on 100\% MAF sample and performance improves for read depths 3089 and 30590. GATK performs well on both the 10\% and 100\% variants, but makes a false positive call at the 100\% MAF level for all read depth levels. VarScan2-pileup performs perfectly for all but the lowest depth for the 100\% MAF.
...
For RVD2, we set $\tau$, the hypothesis test threshold, to the naive value of zero and we optimized $\tau$. RVD2 is able to make use of multiple replicates but the other algorithms do not. So we ran RVD2 with all $N=6$ replicates as well as with $N=1$.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Empirically, RVD2 is able to call variants if the median read depth is $10\times$ the MAF.
...
\subsection{HCC1187 primary ductual carcinoma sample}\label{sec:brca}

We test RVD2 on matched breast tumor-normal sequencing data. The HCC1187 cell line is derived from  epithelial cells from primary breast tissue from a 41 y/o adult with TNM stage IIA primary ductal carcinoma. A matched normal cells were derived from lymphoblastoid cells from whole blood.
...
% TODO: was this copied from somewhere or is this original?

We ran RVD2 on the PAXIP1, a XXkb gene ecoding a protein that is important for maintaining genome stability~\cite{}. We set the paramters of the algorithm to give 4000 gibbs samples with a burn-in of XX\% and a thinning rate of 2. The threshold for the hypothesis tests were $\tau=XX$ and $\alpha=0.05$. 

The algorithm identified XX homozygous germline variants and XX heterozygous germline variants. 
