...
EqlogPmu = -0.5*log(2*pi*sigma20) -(1/sigma20)*(gam2 + (gam1 - u0).^2);
EqlogPmu = repmat(EqlogPmu, [K 1]);

for i = 1:length(gam1)
    EqlogG1(i) = integral(@(x)logG1(x,gam1(i),gam2(i),M0),0,1);
end
for i = 1:length(gam1)
    EqlogG2(i) = integral(@(x)logG2(x,gam1(i),gam2(i),M0),0,1);
end
...
EqlogQmu = -0.5*log(2*pi*exp(1)*gam2);
